...
4. Add the following environment variables under **Environment**:
    ...
    - `GROQ_API_KEY`: Your Groq Cloud API key.
    - `JWT_SECRET`: A secure random string for JWT token generation.
    - `RESEND_API_KEY`: Your Resend API Key (Highly recommended: Render blocks SMTP ports 25, 465, and 587 on the Free Tier, so Resend's HTTP API must be used instead of Gmail SMTP).
