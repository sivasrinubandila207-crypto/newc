 ...
 * Handles email delivery using Brevo SMTP with rate-limiting and custom HTML templates.
 */
...
const transporter = nodemailer.createTransport({
  ...
  family: 4, // Force IPv4 to avoid network resolution timeouts on cloud environments like Render
  connectionTimeout: 15000,
  greetingTimeout: 15000,
  socketTimeout: 20000
});

// Transporter Verification during startup
...
async function sendOTPEmail(email, otp) {
  try {
    ...
    const rawFrom = process.env.EMAIL_FROM || 'sivasrinubandila207@gmail.com';
    const from = rawFrom.includes('<') ? rawFrom : `"InsightDocs AI" <${rawFrom}>`;
    ...
    const mailOptions = {
      from,
      to: email,
      subject: 'InsightDocs AI Verification Code',
      html,
      text
    };
