...
async function sendOTPEmail(to, name, otp, action = 'register') {
  ...
  const BREVO_API_KEY = cleanEnvVar(process.env.BREVO_API_KEY);
  if (BREVO_API_KEY) {
    const fetch = require('node-fetch');
    const rawFrom = process.env.SMTP_FROM || process.env.EMAIL_FROM || 'sivasrinubandila207@gmail.com';
    const fromStr = cleanEnvVar(rawFrom);

    let senderEmail = fromStr;
    let senderName = 'InsightDocs AI';
    const match = fromStr.match(/^(?:"?([^"]*)"?\s)?<([^>]+)>$/);
    if (match) {
      senderName = match[1] || 'InsightDocs AI';
      senderEmail = match[2];
    }

    try {
      console.log(`[EmailService] Sending via Brevo API to ${to}...`);
      ...
      if (!response.ok) {
        throw new Error(resData?.message || resBody || 'Failed to send email via Brevo API');
      }

      console.log(`[EmailService] Email sent successfully to ${to} via Brevo. Message ID: ${resData.messageId}`);
      return { success: true, messageId: resData.messageId };
    } catch (err) {
      console.error(`[EmailService] Failed to send email via Brevo API to ${to}:`, err);
      throw err;
    }
  }
