...
/**
 * Helper to send email using Resend HTTP API.
 */
async function sendViaResend(to, subject, htmlContent, textContent) {
  const apiKey = cleanEnvVar(process.env.RESEND_API_KEY);
  const rawFrom = process.env.SMTP_FROM || process.env.EMAIL_FROM || '';
  let from = rawFrom ? cleanEnvVar(rawFrom) : 'onboarding@resend.dev';

  // Resend has strict validation rules:
  // 1. By default, with sandbox/free tier API keys, you must send FROM onboarding@resend.dev
  // 2. You cannot send FROM personal emails like gmail.com, yahoo.com etc. unless domain is verified.
  // So we sanitize/verify the from address to prevent Resend from throwing errors.
  let finalFrom = from;
  if (from.includes('<') && from.includes('>')) {
    ...
    if (emailMatch) {
      ...
      if (email.endsWith('@gmail.com') || email.endsWith('@yahoo.com') || email.endsWith('@outlook.com') || email.endsWith('@hotmail.com')) {
        ...
        const name = nameMatch ? nameMatch[0].trim() : 'InsightDocs AI';
        finalFrom = `${name} <onboarding@resend.dev>`;
      }
    }
  } else if (from.endsWith('@gmail.com') || from.endsWith('@yahoo.com') || from.endsWith('@outlook.com') || from.endsWith('@hotmail.com') || !from.includes('@')) {
    finalFrom = 'InsightDocs AI <onboarding@resend.dev>';
  }

  console.log(`[EmailService] Sending email via Resend API to ${to} (from: ${finalFrom})`);
  ...
  if (!response.ok) {
    throw new Error(resData.message || `Resend API returned status ${response.status}: ${JSON.stringify(resData)}`);
  }

  console.log(`[EmailService] Email sent successfully via Resend to ${to}. ID: ${resData.id}`);
...
async function sendOTPEmail(to, name, otp, action = 'register') {
  ...
  if (resendApiKey) {
    try {
      ...
      console.error(`[EmailService] Resend API failed:`, err);
      // Fallback to SMTP if SMTP is configured, otherwise rethrow
      const rawUser = process.env.SMTP_USER || process.env.EMAIL_USER || '';
      const rawPass = process.env.SMTP_PASS || process.env.EMAIL_PASS || '';
      if (!cleanEnvVar(rawUser) || !cleanEnvVar(rawPass)) {
        throw err;
      }
      console.warn(`[EmailService] Falling back to SMTP...`);
    }
  }
