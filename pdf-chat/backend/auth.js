...
async function sendOtpForUser(usersCollection, user, otpAction) {
  ...
  let emailResult;
  if (otpAction === 'reset') {
    emailResult = await sendPasswordResetEmail(user.email, otp);
  } else {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'register') {
      ...
      if (existing) {
        if (!existing.isVerified) {
          const emailRes = await sendOtpForUser(users, existing, 'register');
          if (!emailRes.success) {
            const statusCode = emailRes.error.includes('Rate limit') ? 429 : 500;
            return { statusCode, headers: CORS, body: JSON.stringify({ error: emailRes.error }) };
          }
      ...
      const emailRes = await sendOtpForUser(users, newUser, 'register');
      if (!emailRes.success) {
        // Rollback registration on failure to prevent locking user out
    ...
    if (action === 'login') {
      ...
      if (!user.isVerified) {
        const emailRes = await sendOtpForUser(users, user, 'register');
        if (!emailRes.success) {
          const statusCode = emailRes.error.includes('Rate limit') ? 429 : 500;
          return { statusCode, headers: CORS, body: JSON.stringify({ error: emailRes.error }) };
        }
    ...
    if (action === 'resend-otp') {
      ...
      const emailRes = await sendOtpForUser(users, user, otpAction);
      if (!emailRes.success) {
        const statusCode = emailRes.error.includes('Rate limit') ? 429 : 500;
        return { statusCode, headers: CORS, body: JSON.stringify({ error: emailRes.error }) };
      }
    ...
    if (action === 'forgot-password') {
      ...
      const emailRes = await sendOtpForUser(users, user, 'reset');
      if (!emailRes.success) {
        const statusCode = emailRes.error.includes('Rate limit') ? 429 : 500;
        return { statusCode, headers: CORS, body: JSON.stringify({ error: emailRes.error }) };
      }
